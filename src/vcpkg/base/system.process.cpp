...
namespace
{
                                ...
                                STARTUPINFOEXW& startup_info) noexcept
    {
        ...
        if (!CreateProcessW(nullptr,
                            ...
                            &process_info))
        {
            context.report_error(format_system_error_message("CreateProcessW", GetLastError()));
            return false;
        }
        ...
        bool create(DiagnosticContext& context, DWORD dwAttributeCount)
        {
                ...
                GetLastError() != ERROR_INSUFFICIENT_BUFFER)
            {
                context.report_error(
                    format_system_error_message("InitializeProcThreadAttributeList nullptr", GetLastError()));
                return false;
            }
            ...
            {
                context.report_error(
                    format_system_error_message("InitializeProcThreadAttributeList attribute_list", GetLastError()));
                return false;
            }
    ...
    struct AnonymousPipe
    {
        ...
        bool create(DiagnosticContext& context)
        {
            ...
            {
                context.report_error(format_system_error_message("CreatePipe", GetLastError()));
                return false;
            }
    ...
    struct OverlappedOutputPipe
    {
        ...
        bool create(DiagnosticContext& context, std::int32_t debug_id)
        {
            ...
            {
                context.report_error(format_system_error_message("CreateNamedPipeW stdin", GetLastError()));
                return false;
            }
            ...
            {
                context.report_error(format_system_error_message("CreateFileW stdin", GetLastError()));
                return false;
            }
    ...
    struct AnonymousPipe
    {
        ...
        bool create(DiagnosticContext& context)
        {
            ...
            if (pipe(pipefd))
            {
                context.report_error(format_system_error_message("pipe", errno));
                return false;
            }
            ...
            for (size_t idx = 0; idx < 2; ++idx)
            {
                ...
                {
                    context.report_error(format_system_error_message("fcntl", errno));
                    return false;
                }
            }
            ...
            {
                context.report_error(format_system_error_message("pipe2", errno));
                return false;
            }
#endif // ^^^ !Apple
    ...
    struct PosixSpawnFileActions
    {
        ...
        bool adddup2(DiagnosticContext& context, int fd, int newfd)
        {
            ...
            if (error)
            {
                context.report_error(format_system_error_message("posix_spawn_file_actions_adddup2", error));
                return false;
            }
    ...
    struct PosixPid
    {
        ...
        Optional<int> wait_for_termination(DiagnosticContext& context)
        {
            ...
            if (pid != -1)
            {
                ...
                if (child != pid)
                {
                    context.report_error(format_system_error_message("waitpid", errno));
                    return nullopt;
                }
    ...
    void cmd_execute_background(const Command& cmd_line)
    {
        ...
        if (error)
        {
            Debug::print(fmt::format("{}: cmd_execute_background() failed: {}\n",
                                     debug_id,
                                     format_system_error_message("posix_spawn", errno)));
        }
#endif // ^^^ !_WIN32
    }
...
namespace
{
    ...
    struct ChildStdinTracker
    {
        ...
        Optional<bool> do_write(DiagnosticContext& context, int target)
        {
            ...
            if (this_write != 0)
            {
                ...
                {
                    context.report_error(format_system_error_message("write", errno));
                    return nullopt;
                }
                                                                ...
                                                                uint32_t debug_id)
    {
        ...
        {
            context.report_error(format_system_error_message("WriteFileEx", ERROR_INSUFFICIENT_BUFFER));
            return nullopt;
        }
...
namespace vcpkg
{
    Optional<ExitCodeIntegral> cmd_execute_and_stream_data(DiagnosticContext& context, const Command& cmd,
                                                            const std::function<void(StringView)>& data_cb)
    {
        RedirectedProcessLaunchSettings default_redirected_process_launch_settings;
